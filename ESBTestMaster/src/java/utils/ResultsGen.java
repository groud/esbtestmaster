...
public class ResultsGen {
    ...
    /**
     * A asks commands to generate a fake result file
     * @param args
     */
        ...
        try {
            ...
            if (file.isDirectory()) {
                ...
                for (i = 0; i < 5; i = i + 1) {
                    for (j = 1; j < 6; j++) {
                        if (j == 4 || j == 5) {
                            writeEvent(writer, (i * 5) + j, i + 1, true);
                        } else {
                            sum += (writeEvent(writer, (i * 5) + j, i + 1, false));
                        }
                    }
    ...
    /**
     * Returns the response time of the created event set
     * @param writer A writer
     * @param i The request id
     * @param agentId The agent id
     * @param lost If the request should be lost
     * @return
     */
    public static int writeEvent(PrintWriter writer, int i, int agentId, boolean lost) {
        int result = 0;
